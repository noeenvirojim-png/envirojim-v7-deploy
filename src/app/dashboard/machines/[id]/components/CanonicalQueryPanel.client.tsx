...
export function CanonicalQueryPanel({ machineId }: { machineId: string }) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      // Clean up linked items
      const cleanLinked = (items: any[]) =>
        filterParasiticClusters(
          items.map(item => ({ ...item, display_name: item.name }))
        ).map(item => ({ ...item, name: item.display_name }));

      if (data.linked_parts) data.linked_parts = cleanLinked(data.linked_parts);
      if (data.linked_systems) data.linked_systems = cleanLinked(data.linked_systems);
      if (data.linked_maintenance_tasks) data.linked_maintenance_tasks = cleanLinked(data.linked_maintenance_tasks);
      if (data.linked_faults) data.linked_faults = cleanLinked(data.linked_faults);
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <h2 className="text-xl font-bold text-slate-900 dark:text-white mb-2">
                    {result.top_cluster_display || result.top_cluster.canonical_name}
                  </h2>
