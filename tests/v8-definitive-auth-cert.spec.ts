...
test.describe('V8 Production Authentication Certification', () => {
  ...
  test('FULL STACK AUTH CERTIFICATION', async ({ page, context }) => {
    // Phase 1: Browser Initialization (Implicitly handled by Playwright)
    console.log('--- STARTING AUTH CERTIFICATION ---');

    // Phase 2: Login Flow
    await page.goto(`${TARGET_URL}/login`);
    await page.waitForSelector('input[id="email"]', { timeout: 15000 });
    await page.screenshot({ path: 'tests/screenshots/cert_login_pre.png' });
    ...
    await page.click('button:has-text("Authenticate"), button:has-text("Login"), button[type="submit"]');
    ...
    await page.screenshot({ path: 'tests/screenshots/cert_dashboard.png' });
    ...
    const routes = [
      ...
      { name: 'Parts', path: '/dashboard/parts', screenshot: 'cert_parts.png' },
      { name: 'Work Orders', path: '/dashboard/work-orders', screenshot: 'cert_work_orders.png' }
    ];
    ...
    for (const route of routes) {
      ...
      await page.screenshot({ path: `tests/screenshots/${route.screenshot}` });
    }
