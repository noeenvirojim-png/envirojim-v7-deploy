-- ============================================================================
-- ENVIROJIM V6 ENTERPRISE - PERFECT DEPLOYMENT EDITION
-- Complete Security, Audit & RPC Extension + Optional Enhancements
-- ============================================================================
-- 
-- This file extends GOLDEN_V6_ENTERPRISE_SUPREME.sql with:
-- 1. email_templates table for dynamic email management
-- 2. maintenance_rules table for automated scheduling
-- 3. Enhanced seed data for comprehensive testing
--
-- Deploy this file to Supabase SQL Editor for a perfect testing environment.
-- ============================================================================
...
CREATE TABLE public.diagnostic_nodes (
    ...
    options JSONB DEFAULT '[]'::jsonb, -- Array of {label, target_node_id}
...
CREATE TABLE public.diagnostic_sessions (
    ...
    machine_id UUID NOT NULL REFERENCES public.machines(id) ON DELETE CASCADE,
    path JSONB DEFAULT '[]'::jsonb, -- History of chosen nodes
    outcome TEXT,
    deleted_at TIMESTAMPTZ,
    created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ✨ NEW: Maintenance Rules (Automated Scheduling)
...
CREATE TABLE public.checklist_templates (
    ...
    machine_id UUID REFERENCES public.machines(id) ON DELETE CASCADE,
    name VARCHAR(255) NOT NULL,
    items JSONB NOT NULL DEFAULT '[]'::jsonb, -- Array of {label, required}
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- ✨ NEW: Email Templates (Dynamic Email Management)
CREATE TABLE public.email_templates (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    name VARCHAR(100) NOT NULL UNIQUE,
    subject TEXT NOT NULL,
    body_html TEXT NOT NULL,
    variables JSONB DEFAULT '[]'::jsonb, -- Array of variable names like ["userName", "orgName"]
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- V. ROW LEVEL SECURITY (100% COVERAGE)
-- ============================================================================

-- Organizations
ALTER TABLE public.organizations ENABLE ROW LEVEL SECURITY;
CREATE POLICY "org_read" ON public.organizations FOR SELECT USING ((id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "org_write" ON public.organizations FOR ALL USING (public.is_super_admin());

-- Users
ALTER TABLE public.users ENABLE ROW LEVEL SECURITY;
CREATE POLICY "user_read" ON public.users FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "user_write" ON public.users FOR ALL USING (public.is_admin());

-- Sites
ALTER TABLE public.sites ENABLE ROW LEVEL SECURITY;
CREATE POLICY "site_read" ON public.sites FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "site_write" ON public.sites FOR ALL USING (public.is_admin());
...
CREATE POLICY "machine_write" ON public.machines FOR ALL USING (public.is_admin());

-- Parts Catalog
ALTER TABLE public.parts_catalog ENABLE ROW LEVEL SECURITY;
CREATE POLICY "parts_read" ON public.parts_catalog FOR SELECT USING (deleted_at IS NULL);
CREATE POLICY "parts_write" ON public.parts_catalog FOR ALL USING (public.is_admin());

-- Part Requests
ALTER TABLE public.part_requests ENABLE ROW LEVEL SECURITY;
CREATE POLICY "request_read" ON public.part_requests FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "request_write" ON public.part_requests FOR ALL USING ((organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid())) OR public.is_admin());

-- Interventions
ALTER TABLE public.interventions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "intervention_read" ON public.interventions FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "intervention_write" ON public.interventions FOR ALL USING (public.is_admin() OR (organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid())));

-- Intervention Parts
ALTER TABLE public.intervention_parts ENABLE ROW LEVEL SECURITY;
CREATE POLICY "int_parts_read" ON public.intervention_parts FOR SELECT USING (EXISTS (SELECT 1 FROM public.interventions WHERE id = intervention_parts.intervention_id));
CREATE POLICY "int_parts_write" ON public.intervention_parts FOR ALL USING (EXISTS (SELECT 1 FROM public.interventions WHERE id = intervention_parts.intervention_id));

-- Tickets
ALTER TABLE public.tickets ENABLE ROW LEVEL SECURITY;
CREATE POLICY "ticket_read" ON public.tickets FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "ticket_write" ON public.tickets FOR ALL USING (public.is_admin() OR (organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid())));

-- Diagnostics
ALTER TABLE public.diagnostic_nodes ENABLE ROW LEVEL SECURITY;
CREATE POLICY "diag_nodes_read" ON public.diagnostic_nodes FOR SELECT USING (true); -- Publicly readable for auth
CREATE POLICY "diag_nodes_admin" ON public.diagnostic_nodes FOR ALL USING (public.is_admin());

ALTER TABLE public.diagnostic_sessions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "diag_sessions_read" ON public.diagnostic_sessions FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()));
CREATE POLICY "diag_sessions_write" ON public.diagnostic_sessions FOR ALL USING (organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()));

-- Documents
ALTER TABLE public.documents ENABLE ROW LEVEL SECURITY;
CREATE POLICY "doc_read" ON public.documents FOR SELECT USING ((organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin()) AND deleted_at IS NULL);
CREATE POLICY "doc_write" ON public.documents FOR ALL USING (public.is_admin());

-- Maintenance Definitions
ALTER TABLE public.maintenance_definitions ENABLE ROW LEVEL SECURITY;
CREATE POLICY "maint_read" ON public.maintenance_definitions FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
CREATE POLICY "maint_write" ON public.maintenance_definitions FOR ALL USING (public.is_admin());

-- ✨ NEW: Maintenance Rules
ALTER TABLE public.maintenance_rules ENABLE ROW LEVEL SECURITY;
CREATE POLICY "maint_rules_read" ON public.maintenance_rules FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
CREATE POLICY "maint_rules_write" ON public.maintenance_rules FOR ALL USING (public.is_admin());

-- Checklists
ALTER TABLE public.checklist_templates ENABLE ROW LEVEL SECURITY;
CREATE POLICY "tmpl_read" ON public.checklist_templates FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
CREATE POLICY "tmpl_write" ON public.checklist_templates FOR ALL USING (public.is_admin());

ALTER TABLE public.checklists ENABLE ROW LEVEL SECURITY;
CREATE POLICY "chk_read" ON public.checklists FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
CREATE POLICY "chk_write" ON public.checklists FOR ALL USING (organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()) OR public.is_admin());

-- RFQs & Quotes
ALTER TABLE public.rfqs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "rfq_read" ON public.rfqs FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
CREATE POLICY "rfq_write" ON public.rfqs FOR ALL USING (public.is_admin());

ALTER TABLE public.supplier_quotes ENABLE ROW LEVEL SECURITY;
CREATE POLICY "quote_read" ON public.supplier_quotes FOR SELECT USING (EXISTS (SELECT 1 FROM public.rfqs WHERE id = supplier_quotes.rfq_id));
CREATE POLICY "quote_write" ON public.supplier_quotes FOR ALL USING (public.is_admin());

-- Manuals
ALTER TABLE public.manuals ENABLE ROW LEVEL SECURITY;
CREATE POLICY "manual_read" ON public.manuals FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
CREATE POLICY "manual_write" ON public.manuals FOR ALL USING (public.is_admin());

-- ✨ NEW: Email Templates
ALTER TABLE public.email_templates ENABLE ROW LEVEL SECURITY;
CREATE POLICY "email_tmpl_read" ON public.email_templates FOR SELECT USING (true); -- Readable by all authenticated users
CREATE POLICY "email_tmpl_write" ON public.email_templates FOR ALL USING (public.is_admin());

-- Notifications
ALTER TABLE public.notification_logs ENABLE ROW LEVEL SECURITY;
CREATE POLICY "notify_read" ON public.notification_logs FOR SELECT USING (organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) OR public.is_admin());
...
-- VIII. COMPLIANCE VIEWS (Standardized Soft-Delete Filter)
-- ============================================================================
...
-- X. INITIAL SEED (System Root + Test Data)
-- ============================================================================
...
VALUES ('00000000-0000-0000-0000-000000000001', '00000000-0000-0000-0000-000000000001', 'SUPER_ADMIN', 'admin@envirojim.com', 'System Administrator')
ON CONFLICT DO NOTHING;

-- Test Client Organization
INSERT INTO public.organizations (id, name, type, parent_id)
VALUES ('11111111-1111-1111-1111-111111111111', 'Test Client Corp', 'CLIENT', '00000000-0000-0000-0000-000000000001')
ON CONFLICT DO NOTHING;

-- Test Users (for testing different roles)
INSERT INTO public.users (id, organization_id, role, email, full_name)
VALUES 
('22222222-2222-2222-2222-222222222222', '11111111-1111-1111-1111-111111111111', 'ORG_ADMIN', 'org-admin@test.com', 'Test Org Admin'),
('33333333-3333-3333-3333-333333333333', '11111111-1111-1111-1111-111111111111', 'TECHNICIAN', 'tech@test.com', 'Test Technician'),
('44444444-4444-4444-4444-444444444444', '11111111-1111-1111-1111-111111111111', 'OPERATOR', 'operator@test.com', 'Test Operator')
ON CONFLICT DO NOTHING;

-- Test Site
INSERT INTO public.sites (id, organization_id, name)
VALUES ('55555555-5555-5555-5555-555555555555', '11111111-1111-1111-1111-111111111111', 'Main Warehouse')
ON CONFLICT DO NOTHING;

-- Test Machine
INSERT INTO public.machines (id, organization_id, site_id, serial_number, make, model, year, current_hours)
VALUES ('66666666-6666-6666-6666-666666666666', '11111111-1111-1111-1111-111111111111', '55555555-5555-5555-5555-555555555555', 'CAT-12345', 'Caterpillar', 'D6T', 2020, 1500)
ON CONFLICT DO NOTHING;

-- Test Parts
INSERT INTO public.parts_catalog (id, part_number, name, description, price)
VALUES 
('77777777-7777-7777-7777-777777777777', 'P-001', 'Oil Filter', 'Standard oil filter for CAT engines', 45.99),
('88888888-8888-8888-8888-888888888888', 'P-002', 'Air Filter', 'High-efficiency air filter', 89.99),
('99999999-9999-9999-9999-999999999999', 'P-003', 'Hydraulic Fluid', '5-gallon hydraulic fluid', 125.00)
ON CONFLICT DO NOTHING;

-- ✨ NEW: Email Templates (Seed Data)
