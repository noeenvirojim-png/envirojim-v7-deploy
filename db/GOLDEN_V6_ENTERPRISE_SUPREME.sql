-- ============================================================================
-- ENVIROJIM V6 ENTERPRISE - GOLDEN MASTER SCHEMA
...
-- Is Admin? (Checks for ORG_ADMIN or SUPER_ADMIN role)
...
CREATE POLICY "org_read" ON public.organizations FOR SELECT USING (
    id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) 
...
CREATE POLICY "user_read" ON public.users FOR SELECT USING (
    organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) 
    OR public.is_super_admin()
);
CREATE POLICY "user_write" ON public.users FOR ALL USING (
    (organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()) AND public.is_admin())
    OR public.is_super_admin()
);

-- Sites
ALTER TABLE public.sites ENABLE ROW LEVEL SECURITY;
CREATE POLICY "site_read" ON public.sites FOR SELECT USING (
    organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) 
    OR public.is_super_admin()
);
...
CREATE POLICY "machine_read" ON public.machines FOR SELECT USING (
    organization_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) 
    OR assigned_partner_id IN (SELECT org_id FROM public.get_auth_org_hierarchy()) 
    OR public.is_super_admin()
);
CREATE POLICY "machine_write" ON public.machines FOR ALL USING (
    (organization_id = (SELECT organization_id FROM public.users WHERE id = auth.uid()) AND public.is_admin())
    OR public.is_super_admin()
);
