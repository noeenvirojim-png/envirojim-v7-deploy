...
(async () => {
  try {
    ...
    const { data: links, count: lc } = await supabase.from('canonical_cluster_links').select(`id, from_cluster_id, to_cluster_id, link_type, c1:canonical_clusters!from_cluster_id(canonical_name), c2:canonical_clusters!to_cluster_id(canonical_name)`).in('from_cluster_id', cids);
